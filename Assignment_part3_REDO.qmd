...
p_load(devtools,tidyverse,dplyr,ggplot2,latex2exp,stargazer, fixest, modelsummary, knitr, readr, tseries, lmtest, forecast, dynlm, vars, gridExtra, tidy)
...
for (T in seq(init_step, max_T, 1)) {
  ...
  beta_hat <- summary[["coefficients"]][2]
  t_stat <- summary[["coefficients"]][6]
...
We carried out the ADF unit-root test for all 10 time series using the G2S approach based on the Schwarz Information Criterion. This test did not include a trend in the model. The G2S approach finds that including 1 lag in the analysis would fit the time series the most. 
...
```
According to the forecast, we can see that since there is no statistically significant evidence of a drift, the stock of these companies could fluctuate either way. However, Since there is a higher evidence of a positive drift for Apple's stock (looking at the plots of stock prices below) and the p-values of having a positive constant term in the tests above, it is more likely that there exists a positive trend in Apple's stock price. Moreover, the price of Apple's stock seems to be higher than that of Microsoft, so generally speaking, Apple's stock is more valuable than Microsoft's stock.

However, looking at the plots below, the last several periods of Apple's stock seems to follow a downward trends, so it might be that Apple's stock can decrease soon. Thus, I would not dare to give investment advice based solely on this forecast and would check more background information of the companies' current prospects.
...
summary(Micro_Exxon)
```
Based on the simple linear regression above, we find statistically significant evidence of contemporaneous relations between two stock prices. However, since both stock prices are non-stationary I(1) (according to the ADF tests above), this result is not reliable since it is subjected to the spurious regression problems. 
